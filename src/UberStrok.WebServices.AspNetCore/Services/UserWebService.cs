...
namespace UberStrok.WebServices.AspNetCore
{
    public class UserWebService : BaseUserWebService
    {
        ...
        public override async Task<List<ItemInventoryView>> GetInventory(string authToken)
        {
            ...
            var inventory = new List<ItemInventoryView>(member.Inventory.Count);
        ...
        public override async Task<MemberOperationResult> ChangeMemberName(string authToken, string name, string locale, string machineId)
        {
            ...
            if (member == null)
                return MemberOperationResult.MemberNotFound;
            var inventoryItem = member.Inventory.FirstOrDefault(x => x.ItemId == NAME_CHANGE_ID);
        ...
        private static bool CheckMemberInventory(Member member)
        {
            ...
            var expired = new List<InventoryItem>();
            ...
            {
                if (now > vk.Expiration || vk.AmountRemaining == 0)
                    expired.Add(vk);
            }
        ...
        private static bool CheckMemberLoadout(Member member)
        {
            ...
            if (loadout.GearFace != 0 && !inventory.Any(x => x.ItemId == loadout.GearFace))
            {
                loadout.GearFace = 0;
                needUpdate = true;
            }
            if (loadout.GearFace != 0 && !inventory.Any(x => x.ItemId == loadout.GearHead))
            {
                loadout.GearHead = 0;
                needUpdate = true;
            }
            if (loadout.GearGloves != 0 && !inventory.Any(x => x.ItemId == loadout.GearGloves))
            {
                loadout.GearGloves = 0;
                needUpdate = true;
            }
            if (loadout.GearUpperBody != 0 && !inventory.Any(x => x.ItemId == loadout.GearUpperBody))
            {
                loadout.GearUpperBody = 0;
                needUpdate = true;
            }
            if (loadout.GearLowerBody != 0 && !inventory.Any(x => x.ItemId == loadout.GearLowerBody))
            {
                loadout.GearLowerBody = 0;
                needUpdate = true;
            }
            if (loadout.GearBoots != 0 && !inventory.Any(x => x.ItemId == loadout.GearBoots))
            {
                loadout.GearBoots = 0;
                needUpdate = true;
            }
            if (loadout.WeaponMelee != 0 && !inventory.Any(x => x.ItemId == loadout.WeaponMelee))
            {
                loadout.WeaponMelee = 0;
                needUpdate = true;
            }
            if (loadout.WeaponPrimary != 0 && !inventory.Any(x => x.ItemId == loadout.WeaponPrimary))
            {
                loadout.WeaponPrimary = 0;
                needUpdate = true;
            }
            if (loadout.WeaponSecondary != 0 && !inventory.Any(x => x.ItemId == loadout.WeaponSecondary))
            {
                loadout.WeaponSecondary = 0;
                needUpdate = true;
            }
            if (loadout.WeaponTertiary != 0 && !inventory.Any(x => x.ItemId == loadout.WeaponTertiary))
            {
                loadout.WeaponTertiary = 0;
                needUpdate = true;
            }
            if (loadout.QuickItem1 != 0 && !inventory.Any(x => x.ItemId == loadout.QuickItem1))
            {
                loadout.QuickItem1 = 0;
                needUpdate = true;
            }
            if (loadout.QuickItem2 != 0 && !inventory.Any(x => x.ItemId == loadout.QuickItem2))
            {
                loadout.QuickItem2 = 0;
                needUpdate = true;
            }
            if (loadout.QuickItem3 != 0 && !inventory.Any(x => x.ItemId == loadout.QuickItem3))
            {
                loadout.QuickItem3 = 0;
                needUpdate = true;
            }
            if (loadout.FunctionalItem1 != 0 && !inventory.Any(x => x.ItemId == loadout.FunctionalItem1))
            {
                loadout.FunctionalItem1 = 0;
                needUpdate = true;
            }
            if (loadout.FunctionalItem2 != 0 && !inventory.Any(x => x.ItemId == loadout.FunctionalItem2))
            {
                loadout.FunctionalItem2 = 0;
                needUpdate = true;
            }
            if (loadout.FunctionalItem3 != 0 && !inventory.Any(x => x.ItemId == loadout.FunctionalItem3))
            {
