    ...
    public class AuthenticationWebService : BaseAuthenticationWebService
    {
        ...
        public override async Task<MemberAuthenticationResultView> LoginSteam(string steamId, string authToken, string machineId)
        {
            try
            {
                ...
                return await LoginMember(member, machineId);
            }
            catch (Exception ex)
            {
        ...
        private async Task<bool> IsDumbAss(string machineId)
        {
            if (machineId == null)
            {
                return true;
            ...
            int weight = (await _database.Sessions.Where(x => x.HWD == machineId && x.Expiration < DateTime.Now)).Count();
            return weight > 1;
        }

        private async Task<MemberAuthenticationResultView> LoginMember(Member member, string machineId)
        {
            ...
            var authToken = await _sessions.CreateSessionAsync(member, machineId);
        ...
        private async Task<Member> CreateMember(string steamId)
        {
            ...
            int GetDefaultItemId(string prefabName)
            {
                ...
                member.Inventory.Add(new InventoryItem { ItemId = item.Id, AmountRemaining = -1 });
                return item.Id;
            }
        ...
        private async Task<Dictionary<int, int>> CompleteMember(Member member, string name)
        {
            ...
            foreach (var itemPrefabName in _accountConfig.Items)
            {
                ...
                else
                {
                    ...
                    if (!member.Inventory.Any(x => x.ItemId == item.Id))
                        member.Inventory.Add(inventoryItem);
